...
## State definitions for different agents in LangGraph
...
def load_models():
    ...
    llm = ChatGroq(
        ...
        temperature=0.1,
        api_key=os.getenv("GROQ_API_KEY") # API key from environment variable
    )
...
```python
def process_paper(uploaded_file): # defines a function that takes an uploaded file, chunks it, and stores it in a vector store, deleting the temporary file after processing
    """Process uploaded PDF and create vector store"""
    with tempfile.NamedTemporaryFile(delete=False, suffix=".pdf") as tmp_file: # creates a temporary file that behaves like a real file, which stays after closing the file
    ...
    try:
        ...
        documents = list(loader.lazy_load()) # forces loading all pages into memory as a list of Document objects.
        
        # Split text
        text_splitter = RecursiveCharacterTextSplitter(
            chunk_size=1000,
            chunk_overlap=200,
            add_start_index=True # retains the starting position/index of each chunk
        )
        ...
        os.unlink(tmp_file_path) # ensures that the temporary PDF file is deleted from disk, even if an error occurs.
```

# Agents implemented via LangGraph
...
def create_chat_agent():
    def retrieve(state: ChatState):
        if st.session_state.vector_store is None:
            return {"context": []} # creates an empty context if no vector store for context is available
    ...
    def generate(state: ChatState):
        if not state["context"]:
            return {"answer": "Please upload a paper first to chat with it."} # if no context is available, returns a message prompting the user to upload a paper
        
        context_text = "\n\n".join([doc.page_content for doc in state["context"]])
        
        # my prompt template for RAG chat agent
...
def create_comparator_agent():
    def analyze_papers(state: ComparatorState):
        ...
        # Loops over all papers and appends their title and summary in a readable format for the LLM
        papers_info = ""
